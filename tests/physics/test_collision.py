...
def _frictionless_table(**kwargs: float) -> TableConfig:
    """Table with no ball-ball tangential friction (normal impulse lab setup)."""
    return TableConfig(
        rolling_resistance_coefficient=0.0,
        ball_ball_friction=0.0,
        **kwargs,
    )
...
def _touching_pair(
    ...
    *,
    ...
    r = BALL_RADIUS_M
    if table is None:
        table = _frictionless_table(coefficient_of_restitution=1.0)
    overlap = 1e-6
...
def test_head_on_elastic_swaps_velocities() -> None:
    """Head-on contact has no tangential slip, so default mu_bb does not alter the swap."""
    table = TableConfig(rolling_resistance_coefficient=0.0, coefficient_of_restitution=1.0)
    ball_a, ball_b, table = _touching_pair(table=table)
    contacts = find_ball_ball_contacts([ball_a, ball_b])
    assert len(contacts) == 1
...
def _oblique_touching_pair(
    table: TableConfig,
) -> tuple[Ball, Ball, BallBallContact]:
    r = BALL_RADIUS_M
    ...
    contacts = find_ball_ball_contacts([ball_a, ball_b])
    assert len(contacts) == 1
    return ball_a, ball_b, contacts[0]


def test_non_head_on_momentum_conserved() -> None:
    """Oblique contact with default ball-ball friction conserves linear momentum."""
    table = TableConfig(rolling_resistance_coefficient=0.0, coefficient_of_restitution=1.0)
    ball_a, ball_b, contact = _oblique_touching_pair(table)

    p_before = ball_a.mass * ball_a.velocity + ball_b.mass * ball_b.velocity
    resolve_ball_ball_contact(ball_a, ball_b, contact, table)
    p_after = ball_a.mass * ball_a.velocity + ball_b.mass * ball_b.velocity

    np.testing.assert_allclose(p_before, p_after, rtol=1e-5)


def test_non_head_on_friction_dissipates_energy() -> None:
    """Default mu_bb removes kinetic energy on oblique contact."""
    table = TableConfig(rolling_resistance_coefficient=0.0, coefficient_of_restitution=1.0)
    ball_a, ball_b, contact = _oblique_touching_pair(table)
...
def test_non_head_on_elastic_without_ball_friction() -> None:
    """With mu_bb=0, oblique elastic contact conserves kinetic energy."""
    table = _frictionless_table(coefficient_of_restitution=1.0)
    ball_a, ball_b, contact = _oblique_touching_pair(table)
